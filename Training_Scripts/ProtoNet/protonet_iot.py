...
protonet_iot.py
===============
Prototypical Networks (ProtoNet) with MLP backbone
...
Usage:
  python protonet_iot.py [--mode 54class|10class]
                         [--n_way 5] [--k_shot 5] [--q_query 15]
                         [--episodes 5000] [--eval_interval 500]
                         [--lr 0.001] [--device cpu]
...
class EpisodeSampler:
    def __init__(self, data_x, data_y, class_list, n_way, k_shot, q_query, device):
        ...
        assert len(self.valid_classes) >= n_way, \
            f"Not enough valid classes: {len(self.valid_classes)} < {n_way}"
...
def main():
    ...
    print("=" * 72)
    print(f"ProtoNet  |  mode={args.mode}  |  "
    ...
    with open(os.path.join(OUTPUT_DIR,
              f"protonet_feature_names_{suffix}.json"), "w") as fh:
    ...
    best_pt = os.path.join(OUTPUT_DIR, f"protonet_iot_best_{suffix}.pt")

    for ep in range(1, args.episodes + 1):
        model.train()
        sup_x, sup_y, qry_x, qry_y = train_sampler.sample_episode()
        loss, acc = prototypical_loss(
            model(sup_x), sup_y, model(qry_x), qry_y, args.n_way)
        opt.zero_grad()
        loss.backward()
    ...
    for k in [1, 5, 10]:
        ...
        if len(s.valid_classes) < n_way_test:
            continue
        acc, ci = evaluate(model, s, args.eval_episodes * 2, device)
        print(f"  {n_way_test}-way {k:>2}-shot  acc={acc:.4f} ± {ci:.4f}")

    print(f"\n  Best novel acc : {best_novel_acc:.4f}")
    np.save(os.path.join(OUTPUT_DIR,
            f"protonet_iot_history_{suffix}.npy"), dict(history))
